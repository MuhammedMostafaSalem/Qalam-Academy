...
const UpdateProductModal = ({ isOpen, onClose, product, onSuccess }) => {
    ...
    useEffect(() => {
        if (product) {
            setTitleAr(product.title?.ar || product.title || "");
            setTitleEn(product.title?.en || product.title || "");
            setDescriptionAr(product.description?.ar || product.description || "");
            setDescriptionEn(product.description?.en || product.description || "");
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await updateProductAction(product._id, null, formData);
    ...
    return (
        <div onClick={onClose} className="fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center bg-black/60 p-4">
            <div onClick={(e) => e.stopPropagation()} className="bg-card border border-border rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200">
                ...
                <h2 className="text-xl font-bold mb-6 text-text-primary">تعديل المنتج</h2>
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm text-text-secondary mb-1">صورة المنتج (اختياري)</label>
