...
const UpdatePortfolioModal = ({ isOpen, onClose, portfolio, onSuccess }) => {
    ...
    useEffect(() => {
        if (portfolio) {
            setTitleAr(portfolio._translations?.title?.ar || portfolio.title?.ar || (typeof portfolio.title === "string" ? portfolio.title : ""));
            setTitleEn(portfolio._translations?.title?.en || portfolio.title?.en || "");
            setDescriptionAr(portfolio._translations?.description?.ar || portfolio.description?.ar || (typeof portfolio.description === "string" ? portfolio.description : ""));
            setDescriptionEn(portfolio._translations?.description?.en || portfolio.description?.en || "");
            setCategory(portfolio.category?._id || portfolio.category || "");
            setProjectUrl(portfolio.projectUrl || "");
            setGithubUrl(portfolio.githubUrl || "");
            setTechnologies(portfolio.technologies ? portfolio.technologies.join(", ") : "");
            setImage(null);
        }
    }, [portfolio]);
    ...
    const handleSubmit = async (e) => {
        ...
        const result = await updatePortfolioAction(portfolio._id, null, formData);
        
        setIsPending(false);

        if (result.success) {
            successMessage(result.message || (isEn ? "Project updated successfully" : "تم تعديل المشروع بنجاح"));
            if (typeof window !== "undefined") {
                window.dispatchEvent(new Event("portfolio-updated"));
            }
            onSuccess();
        } else {
            errorMessage(result.message || (isEn ? "Failed to update project" : "فشل تعديل المشروع"));
        }
    };
        ...
        <div onClick={onClose} className="fixed inset-0 backdrop-blur-sm z-50 flex items-center justify-center bg-black/60 p-4">
            <div onClick={(e) => e.stopPropagation()} className="bg-card border border-border rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200">
                ...
                <h2 className="text-xl font-bold mb-6 text-text-primary">
                    {isEn ? "Edit Project" : "تعديل المشروع"}
                </h2>
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" value={technologies} onChange={(e) => setTechnologies(e.target.value)} placeholder={isEn ? "e.g. React, Next.js, Tailwind" : "مثال: React, Node.js, Design"} className="w-full h-12 rounded-xl border bg-background px-4 text-text-primary outline-none transition border-border focus:border-primary" />
                    </div>
